...
def preprocess_songs(data_folders):
    ...
    for folder in data_folders:
        for root, _, files in os.walk(folder):
            for file in files:
                ...
                samples, lengths = music_utils.generate_centered_transpose(
                    samples)
                all_samples += samples
                all_lengths += lengths
    ...
    assert (sum(all_lengths) == len(all_samples)
            )  # assert equal number of samples and lengths

    # save all to disk
    print("Saving " + str(len(all_samples)) + " samples...")
    all_samples = np.array(all_samples,
                           dtype=np.uint8)  # reduce size when saving
    all_lengths = np.array(all_lengths, dtype=np.uint32)
    np.save('deep_music/data/interim/samples.npy', all_samples)
    np.save('deep_music/data/interim/lengths.npy', all_lengths)
    print('Done: ', succeeded, 'succeded,', ignored, 'ignored,', failed,
          'failed of', succeeded + ignored + failed, 'in total')
